...
def user2cookie(user, expireDelta):
    '''
...
async def authenticate(*, email, passwd):
    ...
    r = web.Response()
    r.set_cookie(COOKIE_NAME, user2cookie(user, _EXPIRE_DELTA), max_age=_EXPIRE_DELTA, httponly=True)
    user["passwd"] = '********'
...
async def api_register_user(*, email, name, passwd):
    ...
    if not email or not _RE_EMAIL.match(email):
        raise APIValueError('email is not correct')
    if not passwd or not _RE_SHA1.match(passwd):
        raise APIValueError('passwd not correct')
...
async def api_update_blog(id, request, *, name, summary, content):
    ...
    if not blog:
        ...
        raise APIValueError('id', 'cannot find this blog, may be already deleted')
    else:
        if not name or not name.strip():
            raise APIValueError('name', 'name cannot be empty.')
        if not summary or not summary.strip():
...
async def api_comments(*, page='1'):
    ...
    comments = await Comment.findAll(ordrBy='create_at desc', limit=(p.offset, p.limit))
    return dict(page=p, comments=comments)


@post('/api/blogs/{id}/comments/add')
